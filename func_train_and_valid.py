...
def cal_iou(arr1,arr2):
    intersection = np.where((arr1 > 0) & (arr2 > 0),1,0)
    union = np.where((arr1 > 0) | (arr2 > 0), 1, 0)
...
# 这里我们要将训练和验证放在一起 原因是因为我们想从验证的效果中得到最好的模型并保存下来 filename参数指定模型保存路径
def func_train_overlap(model ,dataloaders, criterion, optimizer, scheduler, model_save_path = 'overlap.pth', num_epoch= 10):
    # 记录一下时间
    start_time = time.time()
